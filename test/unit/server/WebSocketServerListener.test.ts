...
import { WebSocketServerListener } from '../../../src/server/WebSocketServerListener';
...
class SimpleWebSocketListener extends WebSocketServerListener {
  public async handleConnection(): Promise<void> {
    // Will be overwritten
  }
}

describe('A WebSocketServerListener', (): void => {
  const logger: jest.Mocked<Logger> = getLoggerFor('mock') as any;
  let server: Server;
  let webSocket: WebSocket;
  let upgradeRequest: HttpRequest;
  let listener: jest.Mocked<SimpleWebSocketListener>;
  ...
  beforeEach(async(): Promise<void> => {
    ...
    listener = new SimpleWebSocketListener() as any;
