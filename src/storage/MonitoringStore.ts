...
const emittedActivities = [ AS.terms.Create, AS.terms.Delete, AS.terms.Update ];
...
export class MonitoringStore<T extends ResourceStore = ResourceStore>
  ...
  private isKnownActivity(term?: Term): term is typeof emittedActivities[number] {
    return Boolean(term && emittedActivities.some((entry): boolean => entry.equals(term)));
  }
}
